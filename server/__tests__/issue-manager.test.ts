...
describe('IssueManager', () => {
  ...
  // ── Rework feedback injection tests ──

  it('injectReworkFeedback replaces agent preamble with rework version', () => {
    setup();
    const command = "hermes chat -q 'You are an autonomous coding agent. First run: curl ...'";
    const feedback = 'VERDICT: CHANGES_REQUESTED\n\nFix the bug in foo()';

    const result = issueManager.injectReworkFeedback(command, feedback);

    expect(result).toContain('working on REWORK');
    expect(result).toContain('Previous review requested these changes');
    expect(result).toContain('<REVIEW_FEEDBACK>');
    expect(result).toContain('Fix the bug in foo()');
    expect(result).toContain('</REVIEW_FEEDBACK>');
    expect(result).toContain('First run: curl');
    expect(result).not.toContain('You are an autonomous coding agent. First');
  });

  it('injectReworkFeedback returns command unchanged for non-standard commands', () => {
    setup();
    const command = "aider --message 'Fix the bug'";
    const feedback = 'Please fix the tests';

    const result = issueManager.injectReworkFeedback(command, feedback);
    expect(result).toBe(command); // unchanged
  });

  it('injectReworkFeedback shell-escapes single quotes in feedback', () => {
    setup();
    const command = "hermes chat -q 'You are an autonomous coding agent. Do the thing.'";
    const feedback = "Don't use eval() — it's dangerous";

    const result = issueManager.injectReworkFeedback(command, feedback);

    // Single quotes in feedback should be shell-escaped as '\''
    expect(result).toContain("Don'\\''t");
    expect(result).toContain("it'\\''s");
    expect(result).not.toContain('You are an autonomous coding agent. Do');
  });

  it('getLatestReviewFeedback returns latest comment body', () => {
    setup();
    ...
    const fakePr = {
      ...
      issueId: 'issue-1',
      comments: [
        { id: 'c1', prId: 'pr-456', author: 'hermes-reviewer', body: 'First review', createdAt: 1000 },
        { id: 'c2', prId: 'pr-456', author: 'hermes-reviewer', body: 'Second review with fixes', createdAt: 2000 },
      ],
    };

    const mockPRManager = {
      getByIssueId: vi.fn().mockReturnValue(fakePr),
      resetToOpen: vi.fn(),
    } as unknown as PRManager;
    issueManager.setPRManager(mockPRManager);

    const feedback = issueManager.getLatestReviewFeedback('issue-1');
    expect(feedback).toBe('Second review with fixes');
  });

  it('getLatestReviewFeedback returns undefined when no PR exists', () => {
    setup();
    ...
    const feedback = issueManager.getLatestReviewFeedback('nonexistent');
    expect(feedback).toBeUndefined();
  });

  it('getLatestReviewFeedback returns undefined when no prManager', () => {
    setup();
    // No prManager set
    const feedback = issueManager.getLatestReviewFeedback('any-id');
    expect(feedback).toBeUndefined();
  });

  it('review→in_progress injects feedback into agent command', () => {
    setup();
    ...
    // Create issue with default hermes agent (has standard preamble)
    const issue = issueManager.create({ title: 'Rework task' });
    fakePr.issueId = issue.id;

    // First cycle: in_progress → review
    issueManager.changeStatus(issue.id, 'in_progress');
    const firstTermId = issueManager.get(issue.id)!.terminalId!;
    const firstCommand = terminalManager.get(firstTermId)!.command;
    expect(firstCommand).toContain('You are an autonomous coding agent.');
    expect(firstCommand).not.toContain('REWORK');

    // Kill terminal and move to review
    terminalManager.kill(firstTermId);
    issueManager.get(issue.id)!.terminalId = null;
    (mockPRManager.getByIssueId as ReturnType<typeof vi.fn>).mockReturnValue(undefined);
    issueManager.changeStatus(issue.id, 'review');

    // Second cycle: review → in_progress (rework) — should inject feedback
    (mockPRManager.getByIssueId as ReturnType<typeof vi.fn>).mockReturnValue(fakePr);
    issueManager.changeStatus(issue.id, 'in_progress');
    const reworkTermId = issueManager.get(issue.id)!.terminalId!;
    const reworkCommand = terminalManager.get(reworkTermId)!.command;

    expect(reworkCommand).toContain('working on REWORK');
    expect(reworkCommand).toContain('Missing error handling in parse()');
    expect(reworkCommand).toContain('<REVIEW_FEEDBACK>');
  });

  it('first in_progress does NOT inject rework feedback', () => {
    setup();
    ...
    const issue = issueManager.create({ title: 'Fresh task' });
    issueManager.changeStatus(issue.id, 'in_progress');

    const termId = issueManager.get(issue.id)!.terminalId!;
    const command = terminalManager.get(termId)!.command;

    // Should have the standard preamble, no rework
    expect(command).toContain('You are an autonomous coding agent.');
    expect(command).not.toContain('REWORK');
  });
});
