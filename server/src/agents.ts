...
export const AGENT_PRESETS: AgentPreset[] = [
  ...
  },
  {
    ...
    name: 'Codex',
    icon: '⬡',
    command: "codex 'You are an autonomous coding agent. If a summarization step occurs, always continue working afterward. First run: curl -s http://localhost:4000/ticket/{{id}}/info to get task details and worktree path. cd into the worktree, complete the task, git commit, then: curl -s -X POST http://localhost:4000/ticket/{{id}}/review'",
  ...
  },
  {
    ...
    name: 'Gemini CLI',
    icon: '✦',
    command: "gemini 'You are an autonomous coding agent. If a summarization step occurs, always continue working afterward. First run: curl -s http://localhost:4000/ticket/{{id}}/info to get task details and worktree path. cd into the worktree, complete the task, git commit, then: curl -s -X POST http://localhost:4000/ticket/{{id}}/review'",
