 ...
 *   Also redirects focus back into the container if it escapes (e.g., when
 *   an element becomes disabled and the browser moves focus to document.body).
 * - On unmount: restores focus to the previously focused element (if it's
 *   still in the DOM).
 *
 * @param containerRef - Ref to the container element that bounds the focus trap.
 * @param enabled - Whether the trap is active. Defaults to true. Use false to
 *   disable trapping without violating the Rules of Hooks (e.g., in DiffViewer
 *   inline mode or ImageWithZoom when not zoomed).
 */
export function useFocusTrap(
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Check if the currently focused element is one of the tracked focusable
      // elements inside the container. If not, focus has either escaped the
      // container entirely (e.g., a button became disabled and the browser moved
      // activeElement to document.body) or is on a non-tab-reachable element
      // inside the container (e.g., the container itself with tabIndex="-1").
      // In either case, redirect focus to the appropriate boundary element.
      const focusableArray = Array.from(focusable);
      const activeIndex = focusableArray.indexOf(document.activeElement as HTMLElement);
    ...
    return () => {
      ...
      // but only if it's still in the DOM. If it was removed (e.g., the
      // trigger element was in a list that re-rendered), .focus() on a
      // detached element is a silent no-op — avoid that and let focus
      // fall to document.body naturally.
